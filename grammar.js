...
module.exports = grammar({
  ...
  rules: {
    ...
    //   when EVENT_NAME [priority N] [timing on|off] { body }
    // EVENT_NAME is a screaming-snake identifier (e.g. CLIENT_ACCEPTED,
    // HTTP_REQUEST, RULE_INIT). The body is a braced TCL script.
    when_event: $ => seq(
      'when',
      field('event', $.event_name),
      repeat(field('modifier', $.event_modifier)),
    ...
    binop_expr: $ => choice(
      ...
      prec.left(PREC.equal_string, seq($._expr, choice(
        'eq', 'ne',
        // iRules string operators (not in stock TCL).
        'starts_with', 'ends_with', 'contains', 'equals',
        'matches', 'matches_regex', 'matches_glob',
      ), $._expr)),
