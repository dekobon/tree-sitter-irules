...
## 8. A Green Test Proves Nothing Until You Have Seen It Fail

A passing test is evidence only when you know what would make it fail. An assertion can be hollow in two independent ways: it targets something other than the thing it names, or it targets the right thing but nothing about the code under test can change its outcome. Both look identical to a green suite, both survive review by reading, and both can sit in the repository for months — the test count goes up, the coverage does not.

**A `# <- ` annotation asserting on the wrong line** (#53, `35cf3e7`). A `# <- keyword` annotation intended to check the `on` in `on event { … }` was placed *after* the block's closing brace. The annotation resolves to the column of its own `#` on the nearest preceding non-assertion line, so it asserted on `}` — captured `@punctuation.bracket @punctuation.delimiter` — and never looked at `on` at all. It stayed green only because `tree-sitter-cli` 0.26.9's assertion comparison was itself defective (fixed upstream in 0.26.10); the routine 0.26.9 → 0.26.12 dependency bump surfaced it as a CI failure and blocked the merge. The test's stated intent was unreachable in any case: at command position `on` is a `simple_word` command name captured `@function`, and the literal `on` keyword exists only as a try-handler introducer, which was already covered elsewhere in the same file.

**A pinning test that pinned a different barrier than it claimed** (`b26f7f2`). The case `try { x } on 42 {r o} { … }` carried a comment stating it would fail if the on-handler `#any-of?` predicate were ever dropped. Deleting the predicate leaves it green. The predicate rejects `"42"`, so the on-handler pattern never matches that node; and even with the predicate gone, the generic `simple_word`/`@number` pattern sits later in the query and wins on the same node. Two independent barriers held the behavior in place and the assertion was blind to both — it pinned the `@number` fall-through, which nothing threatened.

**A false bug report produced by the repro harness** (#31). An early version of the issue claimed that *any* `regexp` pattern beginning with `-` was unparsable. It was an artifact of a harness that omitted the trailing newline: `source_file` requires a `_terminator`, so a newline-less file errors regardless of the rule under test. The real defect was far narrower — only switch-shaped operands following `--`. The harness bug and the grammar bug produced the same `(ERROR)` node.

**Lesson:** Before trusting a test, make it fail on purpose. For a highlight assertion, first confirm *where* it lands — `# <- ` targets the column of the `#` on the nearest preceding non-assertion line, `# ^` the column of the caret — then confirm *what* is there: put a deliberately wrong capture name in the assertion and read the runner's `actual highlights:` output. For any test whose comment claims to guard a specific construct, delete that construct and watch the test go red; if it stays green, the comment is false and should be rewritten to describe what the test actually pins. When a reproduction drives a bug report, make it a complete file with a trailing newline, and confirm the *absence* of the bug on a near-miss input before believing the framing.

---

## 9. A Surviving Mutant Indicts the Test Suite, Not the Code

Deleting a construct and watching the suite stay green is evidence about coverage, not about the construct. Two readings fit that observation equally well: the construct does nothing, or nothing tests the construct. Only the second is supported without further work. Treating a surviving mutant as proof of the first converts a coverage gap into a confident, wrong claim — and once that claim is written into a code comment, it becomes an invitation for the next maintainer to delete load-bearing code.

**The on-handler `.` anchor** (`1b62f8e`, `21ddba7`). Removing the anchor from `(on_handler . (simple_word) @keyword …)` left all 131 corpus tests and 241 highlight assertions passing, and it was written up as inert with a comment saying so. It is load-bearing. `on_handler` is `seq('on', $._concat_word, $.arguments, $._word)`, and `_word` resolves to a bare `simple_word` whenever the body is not braced — so `try {x} on error {r o} continue` gives `on_handler` *two* direct `simple_word` children, the result code and the body. Without the anchor the body word is highlighted `@keyword`. The mutant survived because no test used an unbraced handler body. Worse, two follow-up probes appeared to confirm the "inert" reading, but both used braced bodies: they were generated from the same wrong mental model of the AST instead of from the grammar rule, so they could only agree with it. Compare lesson 3 — the failure is the same one of reasoning from an imagined AST rather than the produced one — and lesson 6, which is this lesson's mirror image at the grammar layer.

**Lesson:** When a mutation leaves the suite green, the only conclusion available is "no test covers this." Resolve the ambiguity by reading the rule that defines the node's children in `grammar.js`, then constructing the input that would distinguish the two readings — not by generating more examples from the model you already hold, which will keep agreeing with itself. If the construct proves load-bearing but cannot be pinned at the layer you are testing — a query anchor whose removal *adds* a capture rather than removing one cannot be caught by a highlight assertion, since assertions can only assert that a capture is present — pin the AST shape it depends on with a corpus test and point the comment at that test by name. Never write "this is redundant" on the strength of a surviving mutant alone.

---

## 10. A Workaround Outlives Its Bug Unless Retiring It Is Part of the Fix

A workaround for an upstream defect is written under pressure and then stops being anyone's problem, because it works. Nothing goes red when the upstream fix ships, so the guard, its comments, and the docs that explain it stay in the tree describing a hazard that no longer exists — the same rot as lesson 5, one layer down: not a false claim about the grammar, but a false claim about the toolchain. The retirement condition has to be written down at the time the workaround lands, or it will never be checked.

**The highlight-test OOM guards** (#32, upstream [tree-sitter/tree-sitter#5645](https://github.com/tree-sitter/tree-sitter/issues/5645)). `tree-sitter test` could spin in an unbounded allocation on a mismatched highlight assertion, OOM-killing the machine. `make test` was wrapped in a `ulimit -v` cap that converted the runaway into a fast `SIGABRT`, and `AGENTS.md` and `README.md` grew notes explaining why bare `tree-sitter test` was forbidden. All of it was correct at the time and all of it became dead weight the moment the pinned CLI carried the fix.

**Four dates, only one of which licenses removal.** The upstream fix merged to master 2026-05-31 ([#5097](https://github.com/tree-sitter/tree-sitter/pull/5097)), was backported to `release-0.26` 2026-06-08 ([#5674](https://github.com/tree-sitter/tree-sitter/pull/5674) — the PR the release notes actually cite), first shipped in `tree-sitter-cli` 0.26.10 on 2026-06-28, and became this project's problem only when `package-lock.json` pinned 0.26.13. An upstream issue closing is not the signal — the signal is the lockfile. Because the lockfile bump (0.26.12, then 0.26.13) was handled as a routine dependency update, it passed through without anyone re-reading what those versions unblocked.

**Half-retirement leaves dangling pointers.** The `README.md` "Known limitations" entry for this hazard had already been deleted in an earlier sweep, but the build instructions still said `make test # runs the suite under a memory cap; see Known limitations` — pointing at a section that no longer said anything about it. Removing the explanation without removing its references produces documentation that is worse than either state.

**The fix changed the failure mode, not the trigger.** `query_testing.rs::parse_position_comments` still reads an assertion marker plus a capture-name-shaped token in *any* comment as an assertion against the preceding line; the upstream fix only made the mismatch report cleanly instead of hanging. So the content rule for highlight-test comments outlived the guard, and the replacement warning comment in `test/highlight/irules_namespaces.irules` — which spelled the markers out literally — failed the suite it was documenting. It had to be rewritten to name the markers in words.

**Lesson:** When you land a workaround for an upstream defect, record the exact retirement condition next to it — the pinned dependency version that makes it unnecessary, not the upstream issue number — and re-read it on every bump of that dependency. When you do retire it, verify the premise by reproducing the original bug under the new version before deleting anything (see lesson 8: the guard's removal is only justified by a fault you have watched behave differently), then grep the whole tree for references rather than deleting the guard alone. Check whether the upstream fix removed the *trigger* or only the *symptom*; if only the symptom, the usage rule stays and must be restated accurately.
