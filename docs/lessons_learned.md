...
**`@variable.builtin` for `$tmm_id` and `static::`** (#13, `c368c99`). The capture `((simple_word) @variable.builtin (#any-of? @variable.builtin "static" "tmm_id"))` matched the bare words `static` and `tmm_id`, but real iRules code uses `$tmm_id` (node type `variable_substitution`) and `static::foo` (which after the #17 fix parses as `(id)` inside `set`). The bare `simple_word` form almost never appears. The fix added captures targeting `(variable_substitution (id) ...)` and `(set (id) ...)` with `#eq?` / `#match?` predicates.
