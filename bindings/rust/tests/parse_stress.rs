//! Stress-corpus integration test.
//!
//! Walks `test/stress/` and asserts the parser produces zero `ERROR`
//! and zero `MISSING` nodes for every file. iRules is a strict
//! superset of TCL, so these synthetic snippets exercise both the
//! inherited TCL surface and the iRules-specific extensions
//! (`when` events, namespaced commands, expression operators).
...
/// Recursively collects every `*.irules` file under `dir`.
fn collect_irules_files(dir: &PathBuf) -> Vec<PathBuf> {
    let mut out = Vec::new();
    if let Ok(entries) = std::fs::read_dir(dir) {
        for entry in entries.flatten() {
            let path = entry.path();
            if path.is_dir() {
                out.extend(collect_irules_files(&path));
            } else if path.extension().and_then(|s| s.to_str()) == Some("irules") {
                out.push(path);
            }
        }
    }
...
fn parses_stress_corpus_with_no_errors() {
    ...
    parser
        .set_language(&tree_sitter_irules::LANGUAGE.into())
        .expect("load iRules grammar");

    let files = collect_irules_files(&stress_dir());
    assert!(
        !files.is_empty(),
